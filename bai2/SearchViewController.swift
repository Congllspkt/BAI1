...
class SearchViewController: UIViewController {
  ...
  @IBOutlet weak var searchBar: UISearchBar!
    ...
    var downloads_Session = URLSession()
  ...
  func startDownload(_ track: Track) {
    ...
    if let urlString = track.previewUrl, let url =  NSURL(string: urlString) {
        ...
        // 2
        download.downloadTask = downloads_Session.downloadTask(with: url as URL)
        // 3
  ...
  func resumeDownload(_ track: Track) {
    ...
    if let urlString = track.previewUrl,
        ...
        if let resumeData = download.resumeData {
            download.downloadTask = downloads_Session.downloadTask(withResumeData: resumeData as Data)
            download.downloadTask!.resume()
            download.isDownloading = true
...
extension SearchViewController: UISearchBarDelegate {
  func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
    ...
    if !searchBar.text!.isEmpty {
        ...
        // 5
        dataTask = downloads_Session.dataTask(with: url! as URL) {
